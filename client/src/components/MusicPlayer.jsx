...
export const PlayListCard = () => {
    ...
    useEffect(() => {
        if (!allSongs) {
            getAllSongs().then((data) => {
                console.log("called getAllSongs", data);
                if (data && data.song) {
                    dispatch({
    ...
    return (
        <div className='absolute left-4 bottom-24 gap-3 py-3 w-350 max-w-[350px] h-510 max-h-[510px] flex flex-col overflow-y-scroll scrollbar-thin rounded-md shadow-md bg-primary'>
            {allSongs.length > 0 ? (
